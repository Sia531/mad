...
mujoco_path = os.path.join(user_folder, ".mujoco", "mujoco210", "bin")
if os.path.exists(mujoco_path):
    if os.name == "posix":
        os.environ["LD_LIBRARY_PATH"] = (
            mujoco_path + ":" + os.environ.get("LD_LIBRARY_PATH", "")
        )
    elif os.name == "nt":
        os.add_dll_directory(mujoco_path)
    os.environ["PATH"] = mujoco_path + os.pathsep + os.environ["PATH"]
    print(f"Added mujoco path to PATH: {mujoco_path}")
else:
    raise Exception(f"Mujoco path not found: {mujoco_path}")
